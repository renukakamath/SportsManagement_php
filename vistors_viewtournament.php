...
if (isset($_GET['tid'])) {
	...
	$q="select * from book_seet where tournament_id='$tid' ";
	$res=select($q);
	if (sizeof($res)>0) {
		alert('alredy booked');
	}else{
	...
	$q="insert into book_seet values(null,'$tid',curdate(),'booked')";
	insert($q);


	alert("inserted successfully");
	return redirect("vistors_viewtournament.php");
	}
...
<center>
    ...
    foreach ($res as $row) {?>
    	<tr>
    		...
    		<td><a class="btn btn-success" href="?tid=<?php echo $row['tournament_id'] ?>"> Book a Seat</a></td>
